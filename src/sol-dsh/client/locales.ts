...
export const zh = {
	...
	observationPackHelp:
		"立即模式下只把 bash / then_run 这类命令转储落盘成预览；read、grep、知识页等保持全文，避免刚读到就被裁掉。需要转储全文时用 read / grep，不要发明 obs_recall。",
	mode: "打包时机",
...
export const en = {
	...
	observationPack: "ObservationPack",
	observationPackHelp:
		"Immediate mode previews only command dumps (bash / then_run). read, grep, and knowledge pages stay in full so the model is not truncated mid-read. Fetch a dump with read/grep — do not invent obs_recall.",
	mode: "Packing timing",
